...
parser.add_argument("--type", default=-1, type=int, help="choose a type 0123")
...
if args.type == 0:
    ...
    args.test_dir = "/data2/ghy/ghy_net/data/thin/test/"
    ...
    with tqdm(total=len(train_loader)) as t:
        for (hazy, clean) in train_loader:
           ...
           # fake_out = DNet(output).mean()
            ...
            total_loss = smooth_loss_l1 + 0.01 * perceptual_loss + 0.0005 * adversarial_loss + 0.5 * msssim_loss_   # 0.01 * perceptual_loss_img + 0.0005 * adversarial_loss_img + 0.5 * msssim_loss_img +
            ...
            t.set_description(
                "===> Epoch[{}] : step_1_loss: {:.2f} step_2_loss: {:.2f} co_loss: {:.2f} G_loss: {:.2f} D_loss: {:.2f} ".format(
                   # epoch, step_1_loss.item(), step_2_loss.item(), co_loss.item(), total_loss.item(), D_loss.item(),
                    epoch, step_1_loss.item(), step_2_loss.item(), total_loss.item(), D_loss.item(),
